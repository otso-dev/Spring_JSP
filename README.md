# Spring_JSP
MVC 패턴은 웹 개발에서 중요한 디자인 패턴입니다. 이 패턴은 애플리케이션을 모델(Model), 뷰(View), 컨트롤러(Controller)의 세 부분으로 나눕니다.

왜냐하면 이렇게 분리함으로써 각 부분의 역할이 명확해지고, 개발과 유지보수가 용이해지기 때문입니다. 모델은 데이터와 비즈니스 로직을 처리하고, 뷰는 사용자 인터페이스를 담당하며, 컨트롤러는 사용자의 입력과 모델의 데이터를 뷰에 전달하는 역할을 합니다.

이러한 분리는 코드의 재사용성과 확장성을 높여줍니다.

왜냐하면 각 부분을 독립적으로 개발하고 테스트할 수 있기 때문입니다.

MVC 패턴은 웹 애플리케이션의 구조를 명확하게 하고, 개발 프로세스를 체계화하는 데 큰 도움을 줍니다.
# Kotlin
Null이 존재하지 않음
